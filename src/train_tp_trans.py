...
fq = Trans(fq_lst, fs_lst, f_q, f_s, padding_mask=None, s_padding_mask=None)
self = Trans
...
if args.crm_type == 'chm':
    ...
    with torch.no_grad():
        weighted_v, corr_out = FusionNet(corr=corr, v=f_s.view(f_s.shape[:2] + (-1,)), ig_mask=ig_mask, ret_corr=True)
...
sim0 = sim[:,(2-1)*60+60-1]               # 只看query image 单点的 sim score的分布

sim0 = torch.max(sim, dim=-1)[0]
...
ig_mask = F.interpolate(ig_mask0.view(1, 1, 60, 60).float(), size=(473, 473), mode='nearest')
...
soft_mask = np.uint8(200* pred_q1.argmax(1).squeeze().numpy())
heatmap = cv2.applyColorMap(cv2.resize(soft_mask, (473, 473)), cv2.COLORMAP_JET)
